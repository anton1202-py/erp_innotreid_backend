...
class CompanySalesSerializer(serializers.ModelSerializer):
    ...
    def get_data(self, obj):
        ...
        if (date_to - date_from).days == 0:
            date = date_from
            date_range = queryset.values("warehouse", "warehouse__oblast_okrugname")
            for warehouse in date_range:
                day_sales = queryset.filter(date__date=date, warehouse=warehouse['warehouse'],company=obj).values('product__vendor_code').annotate(total_sales=Count('id'))
                for sale in day_sales:
                    product_code = sale['product__vendor_code']
                    if product_code not in results:
                        results[product_code] = { warehouse["warehouse__oblast_okrugname"]: 0 for date in date_range}
                    results[product_code][ warehouse["warehouse__oblast_okrugname"]] = sale['total_sales']
